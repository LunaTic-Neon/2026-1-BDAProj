...
REPORT_PATH = Path(__file__).resolve().parents[1] / "보고서.docx"
report_exists = REPORT_PATH.exists()
report_bytes = REPORT_PATH.read_bytes() if report_exists else b""

top_left, top_down = st.columns([0.84, 0.16])
with top_down:
    st.download_button(
        "보고서 다운로드",
        data=report_bytes,
